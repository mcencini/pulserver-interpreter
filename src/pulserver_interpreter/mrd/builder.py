...
class ISMRMRDBuilder:
    ...
    def set_H1resonanceFrequency_Hz(self, gamma: float, B0: float):
        self.experimentalConditions[self.current_encoding].H1resonanceFrequency_Hz = gamma * B0
    ...
    def set_trajectory(
        ...
        for k, v in desc:
            ...
            if isinstance(v, str):
                ...
                ustring.append(el)
        self.encoding[self.current_encoding].trajectoryDescription.userParameterDouble = udouble
        self.encoding[self.current_encoding].trajectoryDescription.userParameterLong = ulong
        self.encoding[self.current_encoding].trajectoryDescription.userParameterString = ustring
    ...
    def set_multiband_info(
        ...
        if calibration_type in xsd.multibandCalibrationType is False:
            raise ValueError("calibration_type must be a valid multibandCalibrationType")
            
        if isinstance(spacing, float):
            spacing = [spacing]
        multiband_spacing = [xsd.multibandSpacingType((np.arange(mb_factor) * dz).tolist()) for dz in spacing]
        ...
        if self.encoding[self.current_encoding].parallelImaging is None:
            self.encoding[self.current_encoding].parallelImaging = xsd.parallelImagingType()
    ...
    def set_diffusion(
        ...
        if bvalue.shape[0] != 1 and bvalue.shape[0] != direction.shape[0]:
            raise ValueError("Number of bvalues must match number of diffusion directions")
        if bvalue.shape[0] == 1:
            bvalue = np.repeat(bvalue, direction.shape[0])
            
        gradDir = [xsd.gradientDirectionType(rl=el[0], ap=el[1], fh=el[2]) for el in direction]
        diffusionParams = [xsd.diffusionType(gradDir[n], bvalue[n]) for n in range(bvalue.shape[0])]
    ...
    def add_user_param(self, name, value):
        if isinstance(value, float):
            if haskey(name, self.userParameters[self.current_encoding].userParameterDouble):
                setparam(name, value, self.userParameters[self.current_encoding].userParameterDouble)
            else:
                el = xsd.userParameterDoubleType(name=name, value=value)
                self.userParameters[self.current_encoding].userParameterDouble.append(el)
                return
        if isinstance(value, int):
            if haskey(name, self.userParameters[self.current_encoding].userParameterLong):
                setparam(name, value, self.userParameters[self.current_encoding].userParameterLong)
            else:
        ...
        if isinstance(value, str):
            if haskey(name, self.userParameters[self.current_encoding].userParameterString):
                setparam(name, value, self.userParameters[self.current_encoding].userParameterString)
            else:
                el = xsd.userParameterStringType(name=name, value=value)
                self.userParameters[self.current_encoding].userParameterString.append(el)
                return
    ...
    def _add_to_waveform(self, name, value):
        ...
        newWaveformHeader = mrd.WaveformHeader(waveform_id=waveId, channels=numChannels, number_of_samples=numSamples)
    ...
    def add_acquisition(
        ...
        if user_int is not None:
            if len(user_int) > len(acq.user_int):
                warnings.warn("Length of provided user_int is larger than ismrmrd.Acquisition.user_int - ignoring extra entries")
            for n in range(len(acq.user_int)):
                acq.user_int = user_int[n]
        if user_float is not None:
            if len(user_float) > len(acq.user_float):
                warnings.warn("Length of provided user_float is larger than ismrmrd.Acquisition.user_float - ignoring extra entries")
